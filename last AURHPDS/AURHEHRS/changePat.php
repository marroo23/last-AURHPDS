<?php
    ...
    if(isset($_POST['submit'])){
        if($_POST['pass']==$_POST['cpass']){
            $un = $_POST['username'];
            $pass = md5($_POST['pass']);
            $slq = mysqli_query($conn, "SELECT * FROM login WHERE role='patient' AND username='$un'");
            if(mysqli_num_rows($slq)>0){
                $sql = mysqli_query($conn, "UPDATE login SET password='$pass' WHERE role='patient' AND username='$un'");
                ?>
                <script>
                alert("Successful.");
                window.location.href = "changePat.php";
            </script>
                <?php
            }
            else{
...
<body>
    <div>
        ...
        <li><h3><a href="patient.php" class="btn btn-info">My Health Record</a></h3></li>
        <li><h3><a href="send_symptom.php" class="btn btn-info">Send Symptom</a></h3></li>
        <li><h3><a href="changePat.php" class="btn btn-info">Set Account</a></h3></li>
