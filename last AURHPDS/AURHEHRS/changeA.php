<?php
    ...
    if(isset($_POST['submit'])){
        if($_POST['pass']==$_POST['cpass']){
            $un = $_POST['username'];
            $pass = md5($_POST['pass']);
            $slq = mysqli_query($conn, "SELECT * FROM login WHERE role='admin' AND username='$un'");
            if(mysqli_num_rows($slq)>0){
                $sql = mysqli_query($conn, "UPDATE login SET password='$pass' WHERE username='$un'");
                ?>
                <script>
                alert("Successful.");
                window.location.href = "changeA.php";
            </script>
                <?php
            }
            else{
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <input type="password" id="un" name="pass" required="" placeholder="enter password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" class="form-control">
                                </div>
                            </div>
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <input type="password" id="un" name="cpass" required="" placeholder="comfirm password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="form-group col-sm-8 col-md-6 col-lg-5">
                        <input type="submit" id="smt" name="submit" class="btn btn-success" value="Submit">
                    </div>
                </form>
    </div>
</body>
